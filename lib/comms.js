...
import { funnel, queue, responseTimes, rate, median, shortDate } from "./derive.js";
...
function change(now, before) {
  ...
  if (Math.abs(d) < 0.1) return ", about the same as the week before";
  return d > 0 ? `, up from ${n(before)} the week before` : `, down from ${n(before)} the week before`;
}
...
export function buildUpdate(p, { days = 7, now = Date.now(), hubspot = false } = {}) {
  ...
  if (!f.contacted && !f.replied) {
    ...
    let s = `We reached ${n(f.contacted)} new people${parts.length > 1 ? ` (${parts.join(" and ")})` : parts.length ? ` ${parts[0]}` : ""}${change(f.contacted, fp.contacted)}.`;
    if (f.replied) {
      ...
      s += f.positive ? `, and ${n(f.positive)} ${f.positive === 1 ? "was" : "were"} interested${who ? `: ${who}` : ""}.` : ", none of them a clear yes yet.";
    } else {
      s += " No replies yet.";
    }
    out.push(s);
  ...
  if (liParts.length) notes.push(`On LinkedIn, ${names(liParts, 4)}.`);
  ...
  const launched = p.campaigns.filter((c) => inWin(firstDay.get(`${c.source}:${c.id}`)));
  if (launched.length) {
    const list = launched.map((c) => {
      const sig = signalByKey(c.signal);
      return sig && sig.key !== "unmapped" ? `${sig.name} on ${c.channel === "email" ? "email" : "LinkedIn"}` : `"${c.name}"`;
    });
    out.push(`New this week: we started ${names(list, 4)}.`);
  }
  ...
  if (waiting.length || rt.count) {
    ...
    if (rt.median != null) {
      const h = rt.median;
      s += ` Our median time to answer was ${h < 1 ? `${Math.max(1, Math.round(h * 60))} minutes` : h < 48 ? `${Math.round(h)} hours` : `${Math.round(h / 24)} days`}${h > 4 ? ", and we want that under 4 hours" : ""}.`;
    }
    out.push(s);
  }
