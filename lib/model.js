...
async function fromDb() {
  ...
  return {
    ...
    daily: daily.map((r) => ({
      ...
      connectionsAccepted: r.connections_accepted, messagesStarted: r.messages_started ?? 0,
    })),
...
// The numbers only change when something writes: a sync, a Clay row, a
// webhook, a reply marked done. So the read is cached across requests under
// one tag, and each of those writers clears it (lib/freshness.js). A click
// on a filter then renders from memory instead of making nine round trips
// to Postgres. Ten minutes is the backstop if a writer ever forgets.
export const DATASET_TAG = "dataset";
const cachedDb = unstable_cache(fromDb, ["gtm-dataset-v2"], { tags: [DATASET_TAG], revalidate: 600 });

// Also cached per request, so four components on one page share one read.
export const dataset = cache(async () => {
  if (!dbConfigured()) return { ...seedDataset(), why: "No database connected yet." };
  try {
    const ds = await cachedDb();
